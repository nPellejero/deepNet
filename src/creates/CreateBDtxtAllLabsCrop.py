...
dataPathCrop = commPath+'images-croped-cleaned/'
dataPath = commPath+'images/'
txtPath  = commPath+'redes/txtFiles/BDtxtCropCleanWTest/'
masterCount = 0
maxIm = 255
maxLabs = 100
numLab = 0
...
for myDir in dirNames:
	...
	for num in range(1,step+1):
		lenNum = len(str(num))
		cant0  = 5-lenNum
		pic = "Pictures"+'0'*cant0+str(num)+".png"
		if not os.path.isfile(dataPathCrop+myDir+"/"+pic):
			continue 
		nombres.append(dataPathCrop+myDir+"/"+pic)
		linesMaster.append([lines[num-1]])	
		picCount = picCount + 1
