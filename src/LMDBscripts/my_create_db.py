...
class Hdf5Writer(DbWriter):
    ...
    def __init__(self, **kwargs):
        ...
        if self._dset_limit is not None:
            ...
            print "_max_count"
            print self._max_count
            print "limit"
            print self._dset_limit
        else:
    ...
    def write_batch(self, batch):
        ...
        print "cant labels:"
        cantLabs = len(batch[0][1]) 
        print cantLabs
        # first batch
        if self._db is None:
            self._create_new_file(len(batch),cantLabs )
        ...
        self._create_new_file(len(batch) - split, cantLabs)
        self._db['data'][:] = data_batch[split:]
        self._db['label'][:] = label_batch[split:]
        self._count += len(batch) - split

    def _create_new_file(self, initial_count, cantLabs):
        ...
        print('Creating HDF5 database at "%s" ...' %
        ...
        self._db.create_dataset('label',
                (initial_count,cantLabs),
                maxshape=(self._max_count,cantLabs),
                chunks=True, compression=self._compression, dtype=self.DTYPE)
...
def create_db(input_file, output_dir,
    ...
    if mean_files:
        for mean_file in mean_files:
            if os.path.exists(mean_file):
                print('overwriting existing mean file "%s"!' % mean_file)
            else:
    ...
    print "hdf5_dset_limit"
    print kwargs.get('hdf5_dset_limit')
    ...
    print('Database created after %d seconds.' % (time.time() - start))
...
def _create_lmdb(image_count, write_queue, batch_size, output_dir,
    ...
    while (threads_done < num_threads) or not write_queue.empty():
        ...
        if time.time() - wait_time > 2:
            print('Processed %d/%d' % (images_written, image_count))
            wait_time = time.time()
    ...
    print('%s images loaded' % images_loaded)

    if images_written == 0:
        raise WriteError('no images written to database')
    print('%s images written to database' % images_written)
...
def _fill_load_queue(filename, queue, shuffle):
    ...
    with open(filename) as infile:
        if shuffle:
            print "WELL DONE, YOU ARER SHUFFLING! \n\n\n"
    ...
    print('%s total lines in file' % total_lines)
    if valid_lines == 0:
        raise BadInputFileError('No valid lines in input file')
    print('%s valid lines in file' % valid_lines)
    for key in sorted(distribution):
        print('Category %s has %d images.' % (key, distribution[key]))
...
def _parse_line(line, distribution):
    ...
    path = line[0]
    label = map(lambda x:int(x),line[1:])
    for _id, lab in enumerate(label):
      if lab == 1:
        distribution[str(_id)] += 1
...
def load_image(path):
    ...
    if os.path.exists(path):
        try:
            ...
            raise LoadError, 'IOError: %s' % e.message
    else:
...
def _load_thread(load_queue, write_queue, summary_queue,
    ...
    while not load_queue.empty():
        ...
        image = load_image(path) - 117 # centra los datos en [-127,127]
        # database mean value: 116.64
        if compute_mean:
            image_sum += image
...
def _save_means(image_sum, image_count, mean_files):
    ...
    for mean_file in mean_files:
        if mean_file.lower().endswith('.npy'):
            ...
            print('Unrecognized file extension for mean file: "%s"' % mean_file)
            continue

        print('Mean saved at "%s"' % mean_file)
