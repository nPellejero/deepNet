...
import datasets.drone_soya as dataset
...
n_epochs = 100    # # of iter at starting learning rate
#niter_decay = 0   # # of iter to linearly decay learning rate to zero
lr = 0.0002       # initial learning rate for adam
ntrain = 114425   # # of examples to train on

X_unlab, _ = dataset.load_data()
np.random.shuffle(X_unlab)
print(X_unlab.shape)
vaX = X_unlab[:10000]
vaX = transform(vaX)
...
p_gen_mem = discrim(M, *discrim_params)
p_gen = discrim(gX, *discrim_params)

d_cost_real = bce(p_real, T.ones(p_real.shape)).mean()
d_cost_gen_mem = bce(p_gen_mem, T.zeros(p_gen_mem.shape)).mean()
d_cost_gen = bce(p_gen, T.zeros(p_gen.shape)).mean()
g_cost_d = bce(p_gen, T.ones(p_gen.shape)).mean()

d_cost = d_cost_real + d_cost_gen + d_cost_gen_mem
g_cost = g_cost_d

cost4d = [d_cost, d_cost_real, d_cost_gen, d_cost_gen_mem]
cost4g = [g_cost]
...
print 'COMPILING'
t = time()
_train_g = theano.function([Z], cost4g, updates=g_updates)
_train_d = theano.function([X, M, Z], cost4d, updates=d_updates)
...
n_updates = 0
n_check = 0
n_iter = n_epochs*ntrain/nbatch
n_updates = 0
n_examples = 0
t = time()

nmem = 64 # debe ser menor que nbatch
sep = 100
lmem = nmem * sep
mem = np.ndarray((lmem, nc, npx, npx))
im = 0

for it in range(n_iter):
    epoch = float(it)*nbatch/ntrain
    imb = transform(get_batch(X_unlab, it, nbatch))
    zmb = floatX(np_rng.uniform(-1., 1., size=(len(imb), nz)))

    im = it%lmem
    mem[im] = np.asarray(_gen(zmb[:1]))

    if (it >= lmem):
        indexes = np.arange(it, it + lmem, sep)%lmem
    else:
        indexes = np.arange(0, it + 1, max((it+1)/nmem, 1))[:nmem]

    mmb = transform(mem[indexes,:,:,:])

    costg = _train_g(zmb)
    costd = _train_d(imb, mmb, zmb[:(len(zmb) - indexes.shape[0])])

    print '%.0f %.4f'%(it, epoch)
    print '    G: %.4f'%(float(costg[0]))
    print '    D: %.4f %.4f %.4f %.4f'%(float(costd[0]), float(costd[1]), float(costd[2]), float(costd[3]))

    if it%100 == 0:
        samples = np.asarray(_gen(sample_zmb))
        color_grid_vis(inverse_transform(samples), (14, 14), 'samples/%s/%d_%.4f.png'%(desc, it, epoch))

    if it in map(lambda x: int((n_iter*x) - 1), [.25, .5, .75, 1.]):
