...
def running_batchnorm(X, g=None, b=None, running_u=None, running_s=None, momentum=0.9, a=1., e=1e-8, test_mode=False):
    """
    batchnorm with support for not using scale and shift parameters
    as well as inference values (u and s) and partial batchnorm (via a)
    will detect and use convolutional or fully connected version
    ...
    if X.ndim == 4:
        if test_mode:
            b_u = running_u.dimshuffle('x', 0, 'x', 'x')
            b_s = running_s.dimshuffle('x', 0, 'x', 'x')
        else:
            b_u = T.mean(X, axis=[0, 2, 3]).dimshuffle('x', 0, 'x', 'x')
            b_s = T.mean(T.sqr(X - b_u), axis=[0, 2, 3]).dimshuffle('x', 0, 'x', 'x')
        if a != 1:
        ...
        if test_mode:
            ...
            u_update = momentum * running_u + (1.-momentum) * b_u.flatten()
            s_update = momentum * running_s + (1.-momentum) * b_s.flatten()
            return X, [(running_u, u_update), (running_s, s_update)]
    elif X.ndim == 2:
        if test_mode:
            u = running_u
            s = running_s
        ...
        if test_mode:
            ...
            u_update = momentum * running_u + (1.-momentum) * u
            s_update = momentum * running_s + (1.-momentum) * s
            return X, [(running_u, u_update), (running_s, s_update)]
    else:
        raise NotImplementedError

def running_l1_batchnorm(X, g=None, b=None, running_u=None, running_s=None, momentum=0.9, a=1., e=1e-8, test_mode=False):
    """
    batchnorm with support for not using scale and shift parameters
    as well as inference values (u and s) and partial batchnorm (via a)
    will detect and use convolutional or fully connected version
    ...
    if X.ndim == 4:
        if test_mode:
            ...
            b_u = T.mean(X, axis=[0, 2, 3]).dimshuffle('x', 0, 'x', 'x')
            b_s = T.mean(T.abs_(X - b_u), axis=[0, 2, 3]).dimshuffle('x', 0, 'x', 'x')
        if a != 1:
            b_u = (1. - a)*0. + a*b_u
            b_s = (1. - a)*1. + a*b_s
        X = (X - b_u) / (b_s + e)
...
def running_mapnorm(X, g=None, b=None, running_u=None, running_s=None, momentum=0.9, a=1., e=1e-8, test_mode=False):

    if X.ndim == 4:
        if test_mode:
            b_u = running_u.dimshuffle('x', 'x', 0, 1)
            b_s = running_s.dimshuffle('x', 'x', 0, 1)
        else:
            b_u = T.mean(X, axis=[0, 1],keepdims=True)
            b_s = T.mean(T.sqr(X - b_u), axis=[0, 1], keepdims=True)
        if a != 1:
            b_u = (1. - a)*0. + a*b_u
            b_s = (1. - a)*1. + a*b_s
        X = (X - b_u) / T.sqrt(b_s + e)
        if g is not None and b is not None:
            X = X*g.dimshuffle('x', 'x', 0, 1) + b.dimshuffle('x', 'x', 0, 1)
        if test_mode:
            return X, []
        else:
            u_update = momentum * running_u + (1.-momentum) * b_u.dimshuffle(2,3)
            s_update = momentum * running_s + (1.-momentum) * b_s.dimshuffle(2,3)
            return X, [(running_u, u_update), (running_s, s_update)]
    else:
        raise NotImplementedError
