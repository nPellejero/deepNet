...
from src.gan_reconstructor import Z_reconstructor
import importlib
from sklearn.externals import joblib

class getz_experiment:

    def __init__(self, model_module_file, process_data_fn):
    ...
    def run(self):
        ...
        reconstructor = Z_reconstructor(model, batch_size)
        ...
        for it in xrange(n_iter):
            loss = reconstructor.train_z_on_batch(X_batch)
            ...
            if (it % iter_save == 0) or (it % 1000 == 0 and it < iter_save):
                ...
                joblib.dump(reconstructor.get_Z_value(), 'models/%s/Z_%d.jl' % (desc, it))
        ...
        joblib.dump(reconstructor.get_Z_value(), 'models/%s/Z_last.jl' % (desc))
