...
gen_layers.append(nn.batch_norm(LL.DenseLayer(gen_layers[-1], num_units=500, nonlinearity=T.nnet.softplus), g=None))
gen_layers.append(nn.l2normalize(LL.DenseLayer(gen_layers[-1], num_units=100**2, nonlinearity=T.nnet.sigmoid)))
gen_dat = LL.get_output(gen_layers[-1], deterministic=False)

# specify supervised model
layers = [LL.InputLayer(shape=(None, 100**2))]
...
dataValTest = np.load('../../../data/afew100/afewValBD.npz')
dataX_train = dataTrain['x']
dataX_valid = dataValTest['x'][250:]
dataY_train = dataTrain['y']
dataY_valid = dataValTest['y'][250:]
dataX_test = dataValTest['x'][:250]
dataY_test = dataValTest['y'][:250]
