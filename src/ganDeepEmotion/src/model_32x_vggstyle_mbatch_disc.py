...
def distance(X, Y):
    squared = (X ** 2).sum(1).reshape((X.shape[0], 1)) + (Y ** 2).sum(1).reshape((1, Y.shape[0])) - 2 * X.dot(Y.T)
    return T.sqrt(squared+1e-6)
...
class GAN_model(Model):
    ...
    def discrim_function(self, test_mode, X, w, w2_0, g2_0, b2_0, u2_0, s2_0, w2, g2, b2, u2, s2,
        ...
        img_len = 3*self.img_shape[0]*self.img_shape[1]
        distances = distance(X.reshape((X.shape[0], img_len)), X.reshape((X.shape[0], img_len)))/ float(img_len)
        d2 = distances + T.eye(X.shape[0])
        mins = T.min(d2, axis=1, keepdims=True)
        ...
        y = sigmoid(T.dot(T.concatenate([T.flatten(h5, 2), mins], axis=1), wy))
