...
class GAN_model(Model):
    ...
    def __init__(self,
        ...
        gw  = gifn((gen_dim, ngf*16*h*w), 'gw')
        gg = gain_ifn((ngf*16*h*w), 'gg')
        gb = bias_ifn((ngf*16*h*w), 'gb')
        gu = bnmean_fn((ngf*16*h*w), 'gu')
        gs = bnstd_fn((ngf*16*h*w), 'gs')
        gw1 = gifn((ngf*16, ngf*8, 5, 5), 'gw1')
        gg1 = gain_ifn((ngf*8), 'gg1')
        gb1 = bias_ifn((ngf*8), 'gb1')
        gu1 = bnmean_fn((ngf*8), 'gu1')
        gs1 = bnstd_fn((ngf*8), 'gs1')
        ...
        hh = ww = 4
        dg5 = gain_ifn((hh,ww), 'dg5')
        db5 = bias_ifn((hh,ww), 'db5')
        du5 = bnmean_fn((hh,ww), 'du5')
        ds5 = bnstd_fn((hh,ww), 'ds5')
        dwy = difn((ndf*8, 1), 'dwy')
        dwc = difn((ndf, 10), 'dwc')
        dbc = bias_ifn((10), 'dbc')

        self.trainable_gen_params = [gw, gg, gb, gw1, gg1, gb1, gw2, gg2, gb2, gw3, gg3, gb3, gw4, gg4, gb4, gwx]
        self.trainable_discrim_params = [dw, dw2, dg2, db2, dw3, dg3, db3, dw4, dg4, db4, dg5, db5, dwy]
        self.trainable_classif_params = [dw, dw2, dg2, db2, dw3, dg3, db3, dw4, dg4, db4, dg5, db5, dwc, dbc]

        self.gen_params = [gw, gg, gb, gu, gs, gw1, gg1, gb1, gu1, gs1, gw2, gg2, gb2, gu2, gs2,
                           gw3, gg3, gb3, gu3, gs3, gw4, gg4, gb4, gu4, gs4, gwx]
        self.disclass_params = [dw, dw2, dg2, db2, du2, ds2, dw3, dg3, db3, du3, ds3, dw4, dg4, db4, du4, ds4, dg5, db5, du5, ds5, dwy, dwc, dbc]
        self.discrim_params = [dw, dw2, dg2, db2, du2, ds2, dw3, dg3, db3, du3, ds3, dw4, dg4, db4, du4, ds4, dg5, db5, du5, ds5, dwy]
        self.classif_params = [dw, dw2, dg2, db2, du2, ds2, dw3, dg3, db3, du3, ds3, dw4, dg4, db4, du4, ds4, dg5, db5, du5, ds5, dwc, dbc]
    ...
    def gen_function(self, test_mode, Z, w, g, b, u, s, w1, g1, b1, u1, s1, w2, g2, b2, u2, s2, w3, g3, b3, u3, s3, w4, g4, b4, u4, s4, wx):
        x0, updates0 = running_batchnorm(T.dot(Z, w), g=g, b=b, running_u=u, running_s=s, momentum=self.bn_momentum,
                                         test_mode=test_mode)

        h0 = relu(x0)
        h0 = h0.reshape((h0.shape[0], self.ngf * 16, self.last_shape[0], self.last_shape[1]))
        x1, updates1 = running_batchnorm(deconv(h0, w1, subsample=(2, 2), border_mode=(2, 2)), g=g1, b=b1,
                                         running_u=u1, running_s=s1, momentum=self.bn_momentum, test_mode=test_mode)
        h = relu(x1)
    ...
    def gen_function_partial(self, layer, H, w, g, b, u, s, w1, g1, b1, u1, s1, w2, g2, b2, u2, s2, w3, g3, b3, u3, s3, w4, g4, b4, u4, s4, wx):
        if layer==1:
            h0 = H
            x1, updates1 = running_batchnorm(deconv(h0, w1, subsample=(2, 2), border_mode=(2, 2)), g=g1, b=b1,
                                             running_u=u1, running_s=s1, momentum=self.bn_momentum, test_mode=True)
        if layer<=2:
            if layer==2:
                h1 = H
            else:
                h1 = relu(x1)
            x2, updates2 = running_batchnorm(deconv(h1, w2, subsample=(2, 2), border_mode=(2, 2)), g=g2, b=b2,
    ...
    def discrim_function(self, test_mode, X, w, w2, g2, b2, u2, s2, w3, g3, b3, u3, s3, w4, g4, b4, u4, s4, g5, b5, u5, s5, wy, wc, bc):
        ...
        h5 = pool_2d(x4, (4,4), ignore_border=True, st=None, padding=(0, 0), mode='max')
